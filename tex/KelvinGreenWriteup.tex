...
\]
The contour contribution requires evaluation of the Green's function near $z=0$ to determine $q$ and \textit{on} $z=0$ to evaluate the free surface elevation $\zeta = -\frac{U}{g}\partial_x\phi$. In the next section we show that the point-source kernel is ill-posed in this limit.

\section{Point-Source Asymptotics and the Breakdown of Linear Theory}\label{sec:point_source_asymptotics}
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{make_figures/point_spectrum.png}
...
Second, even for finite submergence, while the exponential factor provides damping that suppresses the large-$t$ contributions, the peak wavenumber can easily become unresolvable for small $|z|$. In terms of $k$, the wave elevation spectrum $S^2_\zeta = |\frac{U}{g} \partial_x W|^2$ for large $k$ at depth $z < 0$ is
%
\begin{equation}
...
\section{Line-Integrated Kelvin Potentials and Flat-Ship Theory}\label{sec:line_integrated_kernels}
...
\section{Numerical Evaluation by Contour Deformation}\label{sec:fast_evaluator}
...
Values on the order of $\Delta g \approx 5$--$6$ provide accurate evaluations with modest quadrature cost for the point-source kernel. The speed-up relative to a direct quadrature on the real line depends on the depth $z$: for $z=0\^-$ the point-source kernel has no damping and direct quadrature fails, while for small negative $z$ the contour-deformation method provides a speedup of $10^3$--$10^4$ over direct quadrature with relative errors less than $10^{-8}$.
...
\]
The scaled Hankel functions $H_1^{(1,2)}$ are slowly varying and the exponential factors can be absorbed into the complex phase. The line-integrate kernel is therefore expressed as a sum of two oscillatory integrals with shifted phases $g(x,y \pm b)$. The same partitioning strategy is applied to each phase function to establish the slowly varying ranges. These ranges are then merged, direct quadrature is used on the real line using the Bessel prefactor weighting (which always covers $k=0$ where the Hankel functions are singular), and the Hankel contributions are added together on the semi-infinite tails using the appropriate phase function. The only additional complication is that the Hankel functions have branch cuts along the negative real axis, so the contour deformation must be designed to avoid crossing these cuts. Overall, the line-integral evaluation is around twice as expensive as the point-source evaluation, but provides $10^4$--$10^5$ speedup over direct quadrature on $z=0$.
...
\]
and using the same Hankel function decomposition for contour deformation. The fast $t^{-6}$ decay of $A_w$ means direct quadrature is also practical.
